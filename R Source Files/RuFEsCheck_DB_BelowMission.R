...
funcLoadRuFEs_ByCat <- function(dbPath, Grouping, Yr, abMission){
  ...
  sqlQuery(channel, paste0("SELECT Sum(rutDailyCatchByStream.StockCatch) AS SumOfStockCatch, 
                                   cmt.cmtManagementGroups.GroupName, 
                            ...
                            FROM (((rutDailyCatchByStream LEFT JOIN (SELECT cmtStocks.*, 
                              ...
                              AS cmt ON rutDailyCatchByStream.DFOCode = cmt.DFOCode) 
INNER JOIN cmtGearTypes ON rutDailyCatchByStream.cmtGearId = cmtGearTypes.GearTypeID) 
INNER JOIN cmtFisheryUsers ON rutDailyCatchByStream.cmtFisheryUserId = cmtFisheryUsers.FisheryUserID) 
INNER JOIN cmtFisheryTypes ON rutDailyCatchByStream.cmtFisheryTypeId = cmtFisheryTypes.FisheryTypeID
WHERE (((rutDailyCatchByStream.Year)=", Yr, ") AND ((rutDailyCatchByStream.abMission)='", abMission, "'))
GROUP BY cmt.cmtManagementGroups.GroupName, 
                           rutDailyCatchByStream.abMission,
                           rutDailyCatchByStream.Area, 
                           rutDailyCatchByStream.DataType, 
                           rutDailyCatchByStream.Year, 
...
datRF <- funcLoadRuFEs_ByCat(dbPath = db_path, Grouping = "DNA Group", 
                    Yr = year, abMission = "b") %>% 
  tidyr::drop_na(SumOfStockCatch) %>% 
  rownames_to_column("RowNum") %>% 
  dplyr::select(c(RowNum, Year, FName, FisheryCode, FGUserCode, GearTypeCode, 
                  CatchDate, Area, GroupName, SumOfStockCatch)) %>%
  mutate(RF_Date = force_tz(CatchDate, tz = "UTC")) %>% 
