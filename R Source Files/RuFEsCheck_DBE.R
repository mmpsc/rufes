...
datDBE <- readxl::read_excel(paste0(here(), "/Data/rutDataSource_DBE.xlsx"),
                             sheet = "Sheet1")

# Broodstock
datBrood <- readxl::read_excel(paste0(here(), "/Data/Broodstock.xlsx"), sheet = "Sheet1", skip = 1)
...
datSEF <- readxl::read_excel(paste0(here(), "/Data/ruqDataSource_DailyCatch_SEFOutputFINAL_Incl2015.xlsx"),
...
datEsc %<>%
  mutate(SumOffemales_brood = as.numeric(SumOffemales_brood), 
         SumOfmales_brood = as.numeric(SumOfmales_brood)) %>% 
  rowwise() %>% 
...
dat %<>% 
  ...
  select(c(Year, DFOCode, Stream, `MA-RSA Stocks`, DNAgrp, RF_Escapement, 
           RF_EscBrood, TotalBroodstockAdults,
