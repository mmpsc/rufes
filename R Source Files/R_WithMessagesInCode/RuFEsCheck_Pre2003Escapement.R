...
# SPECIFY YEAR
year <- 2002
# specify threshold for percent differences (1E-4 default)
threshold <- 1E-4
...
datRFbelow <- readxl::read_excel(paste0(year, "ruqDataSource_DailyCatchBelowGrouped.xlsx"),
                                 sheet = "ruqDataSource_DailyCatch")
datRFabove <- readxl::read_excel(paste0(year, "ruqDataSource_DailyCatchAboveGrouped.xlsx"),
                                 sheet = "ruqDataSource_DailyCatch")
datRFpass <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassageGrouped.xlsx"),
                                sheet = "ruqDataSource_DailyPassage")

# Escapement data exported from RuFEs database
datEsc <- readxl::read_excel(paste0(year, "ruqDataSource_AnnualEscapement.xlsx"),
                             sheet = "ruqDataSource_AnnualEscapement")
...
datI$GroupName2 <- with(datI, ifelse(is.na(GroupName) & Stream == "NonFraser", 
                                       GroupName2 <- "NonFraser", 
...
# Organize RuFEs data -----
datRFbelow %<>% 
  dplyr::select(Year, GroupName, SumOfStockCatch) %>% 
  dplyr::rename(RF_CatchBelow = SumOfStockCatch) %>% 
  na.omit()
datRFabove %<>% 
  dplyr::select(Year, GroupName, SumOfStockCatch) %>% 
...
setdiff(datRF$GroupName, datIGrped$GroupName) # in RF that aren't in CR, should be 0
setdiff(datIGrped$GroupName, datRF$GroupName)
# check for differences between CR and cmt
setdiff(cmtSub$GroupName, datIGrped$GroupName) # in cmt that aren't in CR
setdiff(datIGrped$GroupName, cmtSub$GroupName) # should be 0
...
datPropEsc %<>%
  ...
  dplyr::summarize(NumStreams = n_distinct(Stream), # same as length(unique())
                                                      ...
                                                      ifelse(pVal > 0 & pZero > 0 & (CatchBelow > 0 | 
                                                             ...
                                                             ifelse((pVal == 1 & CatchBelow == 0 & 
                                                                    ...
                                                                    ifelse(pVal == 1 & (CatchBelow > 0 | 
                                                                           ...
                                                                           DataType <- "Unknown" # currently unknown is basically "no data"
                                                                    )))))))
# add in no data and mix of na/zero data categories, and all NA (even catch/passage) 
...
# Error table ----
d <- datIGrped[rowSums(datIGrped[ , c("CatchBelow", "CatchAbove", "Passage")]) == 0,]
`%!in%` <- negate(`%in%`)

names <- c("stocks in RuFEs not in Input", 
           "stocks in Input not in RuFEs", 
           "stocks in with Input catch/passage not equal to 0 that are not in RuFEs",
