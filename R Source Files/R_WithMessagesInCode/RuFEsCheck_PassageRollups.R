...
# Import data ----
datStream <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassage_DNAGrpByPop.xlsx"),
...
datCU <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassage_ConservationUnit.xlsx"),
                            sheet = "ruqDataSource_DailyPassage")
datDNA <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassage_DNAGrp.xlsx"),
                             sheet = "ruqDataSource_DailyPassage")
datFraMgmt <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassage_FraserPanelMgmtGrp.xlsx"),
                                 sheet = "ruqDataSource_DailyPassage")
datFraStk <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassage_FraserPanelStockGrp.xlsx"),
                                sheet = "ruqDataSource_DailyPassage")
datMA <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassage_MA-RSAStocks.xlsx"),
                            sheet = "ruqDataSource_DailyPassage")
datProdMgmt <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassage_ProductionMgmtGrp.xlsx"),
                                  sheet = "ruqDataSource_DailyPassage")
datProdStk <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassage_ProductionStockGrp.xlsx"),
                                 sheet = "ruqDataSource_DailyPassage")
...
for(i in 1:length(datL)){
  ...
  datL[[i]] <- datL[[i]] %>% 
    tidyr::drop_na(Year) %>% 
    dplyr::select(-c(Date, Population)) # %>% 
  # rename(Stream = Stock) %>%
...
sum(sapply(datCompare, nrow)) == nrow(datLong) # should be TRUE

# example for DNA grp consistency: 
# nrow(datDNA[!is.na(datDNA$Year) == TRUE, ]) == nrow(datL[[2]])
# nrow(datDNA[!is.na(datDNA$Year) == TRUE, ]) == nrow(datGrpd[[2]])

sum(sapply(datL, nrow)) == nrow(datLong) # should be TRUE
which((sapply(datL, nrow) == sapply(datGrpd, nrow)) == FALSE) # should be 0
...
print(paste("Total number of roll-up errors:", nrow(datError)))
...
datTotal %<>% 
  ...
  mutate(TotalRFCatch = sum(SumRFCatch, CatchRemovedInRollup, na.rm = TRUE),
         # check for difference between sum of passage for each grouping calculated from 
         # RuFEs rollups and the sum of passage for each grouping calculated in R
         # can change this threshold for percent difference
         MismatchError = ifelse(pDiff > 0.01 | is.na(pDiff), 
                                MismatchError <- "error", 
                                MismatchError <- "no error"), 
         # check for difference between the total RF passage calculated for each 
         # grouping and the sum of passage for each stream
...
filename <- paste0(here(), "/", year, "/Summaries/PassageRollupSummary_", Sys.Date(),".xlsx")

write.xlsx(summaryTable, filename, sheetName = "Summary Table", 
           row.names = FALSE, col.names = FALSE)
write.xlsx(datTotal, filename, sheetName = "Totaled Passage", 
           row.names = FALSE, append = TRUE)
write.xlsx(mismatchTable, filename, sheetName = "Group Mismatch Summary",
           row.names = FALSE, append = TRUE)
write.xlsx(datError, filename, sheetName = "Passage Rollup Errors",
           row.names = FALSE, append = TRUE)
