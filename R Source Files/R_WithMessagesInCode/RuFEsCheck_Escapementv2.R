...
# Import data ----
# Data sheet from CR100 DataOut files
datCRbelow <- readxl::read_excel(paste0(year, "_CR_2BelowMission_DataOut.xlsm"),
                                 sheet = "DbData", skip = 4)
datCRabove <- readxl::read_excel(paste0(year, "_CR_5AboveMission_DataOut.xlsm"),
                                 sheet = "DbData", skip = 4)
datCRpass <- readxl::read_excel(paste0(year, "_CR_4Passage_DataOut.xlsm"),
                                sheet = "DbData", skip = 4)

# Catch and passage data exported from RuFEs database
datRFbelow <- readxl::read_excel(paste0(year, "ruqDataSource_DailyCatchBelow.xlsx"),
                                 sheet = "ruqDataSource_DailyCatch")
datRFabove <- readxl::read_excel(paste0(year, "ruqDataSource_DailyCatchAbove.xlsx"),
                                 sheet = "ruqDataSource_DailyCatch")
datRFpass <- readxl::read_excel(paste0(year, "ruqDataSource_DailyPassage.xlsx"),
...
datRFbelow %<>% 
  ...
  summarize(RF_CatchBelow = sum(SumOfStockCatch, na.rm = TRUE)) %>% 
  na.omit()
datRFabove %<>% 
  group_by(Year, GroupName) %>% 
  summarize(RF_CatchAbove = sum(SumOfStockCatch, na.rm = TRUE)) %>% 
  na.omit()
datRFpass %<>% 
  group_by(Year, GroupName) %>% 
  summarize(RF_Passage = sum(SumOfStockCatch, na.rm = TRUE)) %>% 
  na.omit()
...
# Error table ----
d <- datCR[rowSums(datCR[ , c("CatchBelow", "CatchAbove", "Passage")]) == 0,]
`%!in%` <- negate(`%in%`)

names <- c("stocks in RuFEs not in CR100", 
           "stocks in CR100 not in RuFEs", 
           "stocks in with CR100 catch/passage not equal to 0 that are not in RuFEs",
